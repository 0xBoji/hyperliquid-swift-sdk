...
@main
struct BasicScheduleCancelExample {
    static func main() async {
        ...
        do {
            ...
            let time = Int64(Date().timeIntervalSince1970 * 1000) + 60000 // 60 seconds from now
            let response = try await client.scheduleCancel(time: time)
            print("Schedule Cancel Response:", response)
        } catch {
            print("Error:", error)
            exit(1)
        }
